...
def jira_config() -> JiraConfig:
    ...
    has_basic_auth = os.environ.get("JIRA_USERNAME") and os.environ.get("JIRA_API_TOKEN")
    has_pat = os.environ.get("JIRA_PERSONAL_TOKEN")
    has_oauth_enable = os.environ.get("ATLASSIAN_OAUTH_ENABLE", "").lower() in ("true", "1", "yes")
    has_full_oauth = all([
    ...
    if not (has_basic_auth or has_pat or has_oauth_enable or has_full_oauth):
        pytest.skip("No valid Jira authentication configuration found - skipping real API tests")
...
def confluence_config() -> ConfluenceConfig:
    ...
    if not confluence_url:
        pytest.skip("CONFLUENCE_URL environment variable not set - skipping real API tests")
    
    # Check for authentication configuration
    has_basic_auth = os.environ.get("CONFLUENCE_USERNAME") and os.environ.get("CONFLUENCE_API_TOKEN")
    ...
    if not (has_basic_auth or has_pat or has_oauth_enable or has_full_oauth):
        pytest.skip("No valid Confluence authentication configuration found - skipping real API tests")
